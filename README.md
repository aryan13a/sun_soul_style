...
### Admin Credentials
- **Username**: `admin`
- **Password**: `SunSoulStyle_SecureAdmin_2026!`
- *Note: For security reasons, you must change this default password via the CMS Dashboard Security tab immediately after your first deploy.*

---

## Vercel Deployment & Environment Variables

When deploying the site to Vercel, the following environment variables must be configured in your Vercel Dashboard:

1. **`SESSION_SECRET`**: A strong, unique random secret (at least 32 bytes) used to sign session cookies. This must be set to ensure admin authentication security.
2. **`BLOB_READ_WRITE_TOKEN`**: The read-write token generated by creating a Vercel Blob store, used for database synchronization (`db.json`) and CMS image uploads.
3. **`RESEND_API_KEY`**: Your API key from Resend, required to securely email password reset links to the admin's contact email.
